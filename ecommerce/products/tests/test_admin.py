...
class TestAttributeAdmin:
    ...
    def test_changelist(self, admin_client: Client):
        ...
        url = reverse("admin:products_attribute_changelist")
    ...
    def test_make_active(
        self, admin_client: Client, attribute: Attribute, admin_user: User
    ):
        """Test that the make_active action is working."""
        url = reverse("admin:products_attribute_changelist")
        response = admin_client.post(
            url, data={"action": "make_active", "_selected_action": [attribute.pk]}
        )
        assert response.status_code == 302

        db_attribute = Attribute.objects.get(pk=attribute.pk)
        assert db_attribute is not None
        assert db_attribute.is_active
        assert attribute.created_by == db_attribute.created_by
        assert admin_user == db_attribute.updated_by
    ...
    def test_make_inactive(
        ...
        url = reverse("admin:products_attribute_changelist")
        response = admin_client.post(
            url, data={"action": "make_inactive", "_selected_action": [attribute.pk]}
        )
        assert response.status_code == 302

        db_attribute = Attribute.objects.get(pk=attribute.pk)
        assert db_attribute is not None
        assert not db_attribute.is_active
        assert attribute.created_by == db_attribute.created_by
        assert admin_user == db_attribute.updated_by

    def test_add(self, admin_client: Client):
        """Test that the add page is accessible."""
        url = reverse("admin:products_attribute_add")
        response = admin_client.get(url)
        assert response.status_code == status.HTTP_200_OK
        ...
        assert response.status_code == 302
        assert Attribute.objects.filter(name="test").exists()

    def test_delete(self, admin_client: Client, attribute: Attribute):
        """Test that the delete page is not accessible."""
        url = reverse(
            "admin:products_attribute_delete", kwargs={"object_id": attribute.pk}
        )
        response = admin_client.post(url)
        assert response.status_code == 403

    def test_edit(self, admin_client: Client, attribute: Attribute):
        """Test that the edit page is accessible."""
        url = reverse(
            "admin:products_attribute_change", kwargs={"object_id": attribute.pk}
        )
        response = admin_client.get(url)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_view_page(self, admin_client: Client, attribute: Attribute):
...
class TestCategoryAdmin:
    ...
    def test_view_page(self, admin_client: Client, category: Category):
        """Test that the view page is accessible."""
        url = reverse(
            "admin:products_category_change", kwargs={"object_id": category.pk}
        )
...
class TestProductAdmin:
    ...
    def test_view_page(self, admin_client: Client, product: Product):
...
class TestBrandAdmin:
    ...
    def test_view_page(self, admin_client: Client, brand: Brand):
        """Test that the view page is accessible.""" ""
        url = reverse("admin:products_brand_change", kwargs={"object_id": brand.pk})
        response = admin_client.get(url)
        assert response.status_code == status.HTTP_200_OK
